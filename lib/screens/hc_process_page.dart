...
import '../features/price_list/screens/add_test_dialog.dart';
...
class _HCProcessPageState extends ConsumerState<HCProcessPage> {
  ...
  Future<void> _loadWorkOrder() async {
    ...
    try {
      ...
      if (doc != null) {
        setState(() {
          _doc = doc;
          _clientMobile = doc['mobile'] ?? '';

          final storage = ref.read(storageServiceProvider);
          _techMobile = storage.getFromSession('logged_in_mobile') ?? '';

          if (doc['b2b_client_id'] != null && doc['b2b_client_id'] != 0) {
            _b2bClient = true;
            _b2bClientDetail = doc['b2b_client_name'] ?? '';
          }

          int credit = doc['credit'] ?? 0;
          if (credit == 1) {
            _creditClient = true;
            _trialClient = false;
          } else if (credit == 2) {
          ...
          if (doc['total'] != null) {
            _billAmount = double.tryParse(doc['total'].toString()) ?? 0;
            _calculateDiscount();
          }
          ...
          if (doc['settings'] != null) {
            _sms = doc['settings']['send_sms'] ?? 0;
            _whatsapp = doc['settings']['send_whatsapp'] ?? 0;
            _email = doc['settings']['send_email'] ?? 0;
          }
        });
      ...
      _showSnackBar('Error loading work order');
    } finally {
  ...
  Future<void> _finishSteps(String mode) async {
    ...
    try {
      ...
      debugPrint('💾 Saving work order to local DB...');
      await ref
          .read(dbHandlerProvider)
          .putDocument('work_orders', widget.workOrderId, _doc!);
      debugPrint('✅ Work order saved to local DB');

      debugPrint('🔍 Verifying document is queued for push...');
      final syncStatus = ref.read(dbHandlerProvider).getSyncStatus();
      debugPrint('Sync status: $syncStatus');

      debugPrint('🔄 Forcing immediate sync to remote...');
      await ref.read(dbHandlerProvider).forceSync('work_orders');
      debugPrint('✅ Forced sync completed');

      final syncStatusAfter = ref.read(dbHandlerProvider).getSyncStatus();
      ...
      debugPrint('⏳ Waiting for sync to propagate...');
  ...
  Future<void> _sendNotification() async {
    try {
      ...
      await ref
          .read(dbHandlerProvider)
          .putDocument('notifications', notification['_id'], notification);
  ...
  Future<void> _uploadPhotoOffline(XFile image) async {
    try {
      ...
      await ref
          .read(dbHandlerProvider)
          .putDocument('temp_db', tempDocId, tempDoc);
